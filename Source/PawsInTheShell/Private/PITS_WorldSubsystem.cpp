...
void UPITS_WorldSubsystem::ChangeCharacter()
{
	...
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), APITS_BaseCharacter::StaticClass(), FoundCharacters);
	...
	if (FoundCharacters.Num() > 1)
	{
		...
		if (AController* Controller = GetWorld()->GetFirstPlayerController())
		{
			...
			if (APITS_BaseCharacter* NewCharacter = Cast<APITS_BaseCharacter>(FoundCharacters[RandomIndex]))
			{
